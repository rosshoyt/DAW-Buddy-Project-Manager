...
const DAWProjectSearcher = require('./controller/daw-project-searcher').DAWProjectSearcher;
...
ipcMain.on('readusersearchpaths', function(e){
    var currUserID = authenticationController.getCurrentUserID();
    var userProfile = databaseController.readUserProfile(currUserID);
    // if(userProfile == null){
    //     userProfile = new UserProfile();
    //     // set userid
    // }
    // if any entries were read, send to the main window
    windowController.mainWindow.webContents.send('updateusersearchpaths', userProfile.getAllSearchDirectories());
});
